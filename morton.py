...
#grid = nestedgrid.WorldMercator()
#grid = nestedgrid.WorldCommensurate()
#grid = nestedgrid.WorldFitting()
grid = nestedgrid.PAIRS()

# Valid range
valid_range = shapely.geometry.box(*grid.valid_range)
...
base_repr = numpy.vectorize(numpy.base_repr)
len_np = numpy.vectorize(len)
int_np = numpy.vectorize(int)

def morton(x_idx: numpy.array, y_idx: numpy.array) -> numpy.array:
...
def morton_inv(z_idx: numpy.array) -> Tuple[numpy.array, numpy.array]:
    """Inverse of morton.
...
def _resolution(level: int) -> Tuple[float, float]:
    ...
    return grid.res0.x * factor, grid.res0.y * factor

def _resolution_np(level: numpy.array) -> Tuple[numpy.array, numpy.array]:
    """Pixel resolution based on level.
...
RESOLUTION_X, RESOLUTION_Y = _resolution_np(numpy.arange(grid.max_levels + 1))

def resolution(level: int) -> Tuple:
    """Pixel resolution based on level."""
    try:
        # Get the resolution from a pre-computed dictionary
        return RESOLUTION_X[level], RESOLUTION_Y[level]
    except KeyError:
        try:
            return _resolution_np(level)
        except AttributeError:
            return _resolution(level)

def resolution_x(level: int):
    """Resolution in x dimension at specified level."""
    try:
        # Get the resolution from a pre-computed dictionary
        return RESOLUTION_X[level]
    except KeyError:
        try:
            return _resolution_np(level)[0]
        except AttributeError:
            return _resolution(level)[0]

def resolution_y(level: int):
    """Resolution in y dimension at specified level."""
    try:
        # Get the resolution from a pre-computed dictionary
        return RESOLUTION_Y[level]
    except KeyError:
        try:
            return _resolution_np(level)[1]
        except AttributeError:
            return _resolution(level)[1]

def x_coord_to_idx(x_coord: numpy.array, level: int) -> numpy.array:
    """Translate x-coordinate (e.g., longitude) to x-index at level."""
    return (x_coord - grid.origin.x) / resolution_x(level)

def y_coord_to_idx(y_coord: numpy.array, level: int) -> numpy.array:
    """Translate y-coordinate (e.g., latitude) to y-index at level."""
    return (y_coord - grid.origin.y) / resolution_y(level)

def x_idx_to_coord(x_idx: numpy.array, level: int) -> numpy.array:
    """Translate x-index at level to x-coordinate (e.g., longitude)."""
    return x_idx * resolution_x(level) + grid.origin.x

def y_idx_to_coord(y_idx: numpy.array, level: int) -> numpy.array:
    """Translate y-index at level to y-coordinate (e.g., latitude)."""
    return y_idx * resolution_y(level) + grid.origin.y

def x_idx_to_center_coord(x_idx: numpy.array, level: int) -> numpy.array:
    """Translate x-index at level to center x-coordinate."""
    return (x_idx + .5) * resolution_x(level) + grid.origin.x

def y_idx_to_center_coord(y_idx: numpy.array, level: int) -> numpy.array:
    """Translate y-index at level to center y-coordinate."""
    return (y_idx + .5) * resolution_y(level) + grid.origin.y

def coordinates_to_xy_cell_indices(
    bounds: Tuple, level: int, delta_pixel_cell: int) -> Tuple[Iterable, Iterable]:
    ...
    north = bounds[2]
    east = bounds[3]
    cellminx = math.floor(x_coord_to_idx(numpy.array([west]), level-delta_pixel_cell))
    cellminy = math.floor(y_coord_to_idx(numpy.array([south]), level-delta_pixel_cell))
    cellminx = max(0, cellminx)
    cellminy = max(0, cellminy)
    cellmaxx = math.ceil(x_coord_to_idx(numpy.array([east]), level-delta_pixel_cell))
    cellmaxy = math.ceil(y_coord_to_idx(numpy.array([north]), level-delta_pixel_cell))
    cellrx = range(cellminx, cellmaxx)
    cellry = range(cellminy, cellmaxy)
    return cellrx, cellry

def get_key(y_coord: numpy.array, x_coord: numpy.array, level: int) -> numpy.array:
    ...
    """
    res_x, res_y = resolution(level)
    x_idx = (x_coord - grid.origin.x) / res_x
    y_idx = (y_coord - grid.origin.y) / res_y
    return morton(x_idx, y_idx)

def coordinates(key: numpy.array, level: int) -> Tuple[numpy.array, numpy.array]:
    ...
    for i in range(level):
        ...
        y_idx+= (key&mask)>>(i+1)
    res_x, res_y = resolution(level)
    x_coord = x_idx * res_x + grid.origin.x
    y_coord = y_idx * res_y + grid.origin.y
    return x_coord, y_coord

def center_coordinates(key: numpy.array, level: int) -> Tuple[numpy.array, numpy.array]:
    ...
    for i in range(level):
        ...
        y_idx+= (key&mask)>>(i+1)
    res_x, res_y = resolution(level)
    x_coord = (x_idx + .5) * res_x + grid.origin.x
    y_coord = (y_idx + .5) * res_y + grid.origin.y
    return x_coord, y_coord

def parent_key(key: numpy.array, levels_up: int) -> numpy.array:
...
def children_keys(key: numpy.array, levels_down: int) -> numpy.array:
...
def key_to_box(key: numpy.array, level: int) -> shapely.Polygon:
    ...
    # Nodes encompass half-open intervals [south,north) and [west,east),
    # so that points (and some lines) are assigned to exactly one box on each resolution level.
    west, south = coordinates(key, level)
    res_x, res_y = resolution(level)
    east = west + res_x - EPSILON
    north = south + res_y - EPSILON
    return shapely.box(west, south, east, north)

def encode(key: numpy.array, level: numpy.array) -> numpy.array:
...
def decode(q_key: numpy.array(str)) -> Tuple[numpy.array, numpy.array]:
    ...
    level = len_np(q_key)-2  # -2 to remove the leading 0q
    try:
        # Try vectorized first for speed
        key = int_np(numpy.char.replace(q_key, '0q', ''), 4)
    except ValueError:
...
def base4_to_xy_indices(q_key: numpy.array(str)) -> Tuple[numpy.array, numpy.array]:
    """Convert the quaternary string to y and x coordinate indices."""
    key, level = decode(q_key)
    max_level = level.max()
    y_idx = 0
    x_idx = 0
...
def base4_to_coords(q_string: numpy.array(str)) -> Tuple[numpy.array, numpy.array]:
    """Translate base4 quaternary number to xy coordinate pair."""
    key, level = decode(q_string)
    max_level = level.max()
    y_idx = 0
    x_idx = 0
    for i in range(max_level):
...
def base4_to_center_coords(q_string: numpy.array(str)) -> Tuple[numpy.array, numpy.array]:
...
def base4_to_box(q_string: numpy.array(str)) -> numpy.array(shapely.Polygon):
    """Translate keys to shapely boxes.
    ...
    # so that points (and some lines) are assigned to exactly one box on each resolution level.
    res_x, res_y = resolution(level)
    south = y_idx * res_y + grid.origin.y
    west = x_idx * res_x + grid.origin.x
    east = west + res_x - EPSILON
    north = south + res_y - EPSILON
    return shapely.box(west, south, east, north)
